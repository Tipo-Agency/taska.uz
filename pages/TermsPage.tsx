...
const TermsPage: React.FC = () => {
  ...
  return (
    <MarketingPageShell
      accent="slate"
      kicker={t('legal.terms.kicker')}
      title={t('legal.terms.title')}
      lead={t('legal.terms.updated')}
      heroAlign="left"
      compact
      contentMaxWidth="3xl"
    >
      <div className="space-y-5 md:space-y-6">
        ...
        {SECTION_KEYS.map((k, i) => (
          ...
          >
            <h2 className="text-lg md:text-xl font-bold text-ink mb-3">{t(`legal.terms.${k}Title`)}</h2>
            <p className="text-ink-muted leading-relaxed text-[15px] md:text-base">{t(`legal.terms.${k}Body`)}</p>
          </motion.section>
        ))}
