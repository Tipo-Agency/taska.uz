...
import { ClientsModuleHeroVisual } from './ClientsModuleHeroVisual';
...
const FEATURE_ICONS = [Building2, FileText, Wallet, Search, Tag, Landmark] as const;

const OTHER_MODULES: ModuleId[] = MODULE_IDS.filter((m) => m !== 'clients');

type ClientsModuleLandingProps = {
  onOpenModal: () => void;
};

export const ClientsModuleLanding: React.FC<ClientsModuleLandingProps> = ({ onOpenModal }) => {
  const { t } = useLanguage();
  const included = [
    t('modules.clients.b1'),
    t('modules.clients.b2'),
    t('modules.clients.b3'),
    t('modules.clients.b4'),
  ];
  ...
  const viewCards = [
    {
      icon: Users,
      titleKey: 'modules.clients.page.viewBaseTitle' as const,
      leadKey: 'modules.clients.page.viewBaseLead' as const,
      accent: 'from-violet-600 to-purple-700',
    },
    {
      icon: FileText,
      titleKey: 'modules.clients.page.viewContractsTitle' as const,
      leadKey: 'modules.clients.page.viewContractsLead' as const,
      accent: 'from-slate-600 to-slate-800',
    },
    {
      icon: Wallet,
      titleKey: 'modules.clients.page.viewFinanceTitle' as const,
      leadKey: 'modules.clients.page.viewFinanceLead' as const,
      accent: 'from-emerald-600 to-teal-700',
    },
    {
      icon: Landmark,
      titleKey: 'modules.clients.page.viewDebtsTitle' as const,
      leadKey: 'modules.clients.page.viewDebtsLead' as const,
  ...
  return (
    <main className="bg-canvas">
      <section className="relative min-h-[min(100vh,56rem)] flex flex-col justify-center pt-28 pb-16 md:pb-24 overflow-hidden bg-canvas bg-mesh-canvas">
        ...
        <div className="absolute top-1/4 -left-24 w-[420px] h-[420px] bg-purple-400/12 blur-[100px] rounded-full" />
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="max-w-4xl mx-auto text-center mb-6 md:mb-10">
            ...
            >
              {t('modules.clients.page.heroKicker')}
            </motion.p>
            <motion.div
            ...
            >
              <span className="inline-flex items-center gap-2 text-sm font-semibold px-4 py-1.5 rounded-full border border-purple-200/80 bg-purple-50/95 text-purple-950 shadow-sm">
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          <motion.div {...fadeUp} className="relative rounded-[2rem] overflow-hidden">
            ...
            <div className="relative border border-slate-200/80 bg-gradient-to-br from-slate-50/90 to-white rounded-[2rem] p-8 md:p-12 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.2)]">
              ...
              <p className="text-lg md:text-xl text-ink leading-relaxed font-medium">{t('modules.clients.whoFor')}</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 md:py-28 bg-gradient-to-b from-white to-slate-50/80">
        <div className="container mx-auto px-4">
          <motion.div {...fadeUp} className="max-w-3xl mx-auto text-center mb-12 md:mb-16">
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-violet-700 mb-3">{t('modules.clients.page.viewsKicker')}</p>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 tracking-tight mb-4">{t('modules.clients.page.viewsTitle')}</h2>
            <p className="text-gray-500 text-lg leading-relaxed">{t('modules.clients.page.viewsLead')}</p>
          </motion.div>
      ...
      <section className="py-20 md:py-28 bg-white border-t border-slate-200/60">
        <div className="container mx-auto px-4">
          <motion.div {...fadeUp} className="max-w-2xl mb-10 md:mb-14">
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500 mb-3">{t('modules.clients.page.detailKicker')}</p>
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 tracking-tight mb-3">{t('modules.clients.page.detailTitle')}</h2>
            <p className="text-gray-500 text-lg">{t('modules.clients.page.detailLead')}</p>
          </motion.div>
          ...
          >
            <div className="px-4 py-3 border-b border-slate-200/80 bg-white flex items-center justify-between">
              <span className="text-sm font-bold text-slate-900">{t('modules.clients.demo.modalTitle')}</span>
              <span className="text-slate-400">×</span>
            </div>
            <div className="grid lg:grid-cols-1 divide-y divide-slate-200/80 bg-white">
              <div className="p-4 md:p-5">
                <div className="flex flex-wrap rounded-lg bg-slate-100 p-0.5 gap-0.5 mb-5">
                  {[t('modules.clients.demo.tabCompany'), t('modules.clients.demo.tabNotes'), t('modules.clients.demo.tabContractsModal')].map((label, i) => (
                    <span
                      key={label}
                      className={`px-3 py-2 rounded-md text-[10px] sm:text-xs font-bold ${
                        i === 2 ? 'bg-white text-slate-900 shadow-sm border border-slate-200/80' : 'text-slate-500'
                      }`}
                    >
                      {label}
                    </span>
                ...
                <div className="rounded-xl border border-slate-200/80 bg-slate-50/50 p-4 mb-3">
                  <p className="text-[10px] font-bold text-slate-500 uppercase mb-2">{t('modules.clients.demo.contractsSection')}</p>
                  <div className="flex items-start justify-between gap-3 rounded-lg bg-white border border-slate-200/80 p-3">
                    <div>
                      <span className="text-[10px] font-bold text-violet-600">1</span>
                      <p className="text-sm font-bold text-slate-900 mt-0.5">{t('modules.clients.demo.modalContractService')}</p>
                      <p className="text-lg font-bold text-emerald-600 tabular-nums mt-1">{t('modules.clients.demo.modalContractAmount')}</p>
                    </div>
                ...
                <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/30 px-3 py-4 text-center text-sm text-slate-400">
                  {t('modules.clients.demo.salesEmpty')}
                </div>
                <div className="flex flex-wrap items-center justify-between gap-3 mt-5 pt-4 border-t border-slate-100">
                  <button type="button" className="text-sm font-semibold text-rose-600">
                    {t('modules.clients.demo.deleteBtn')}
                  </button>
                  <div className="flex gap-2">
                    <button type="button" className="text-sm font-semibold text-slate-600 px-3">
                      {t('modules.clients.demo.cancelBtn')}
                    </button>
                    <Button className="!bg-violet-600 hover:!bg-violet-700 !border-none text-sm px-5">{t('modules.clients.demo.saveBtn')}</Button>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="py-20 md:py-28 bg-slate-50 border-t border-slate-200/60">
        <div className="container mx-auto px-4">
          <motion.div {...fadeUp} className="max-w-2xl mb-12 md:mb-16">
            ...
            <p className="text-gray-500 text-lg">{t('modules.clients.page.featuresLead')}</p>
          </motion.div>
            ...
            {FUNC_KEYS.map((k, i) => {
              ...
              return (
                <motion.div
                  ...
                  className="group rounded-3xl border border-gray-200 bg-gradient-to-br from-white to-violet-50/35 p-6 md:p-8 hover:border-violet-200 hover:shadow-xl transition-all duration-300"
                >
                  <div className="inline-flex p-3 rounded-2xl bg-violet-100 text-violet-900 mb-5 group-hover:scale-105 transition-transform">
                    <Icon size={24} />
                  </div>
                  <p className="text-gray-800 leading-relaxed text-[15px] md:text-base font-medium">{t(`modules.clients.${k}`)}</p>
                </motion.div>
              );
            })}
          </div>
      ...
      <section className="py-20 md:py-28 bg-gradient-to-b from-white to-slate-50/50 border-t border-slate-200/60">
        <div className="container mx-auto px-4">
          <motion.div {...fadeUp} className="max-w-2xl mb-10 md:mb-14">
            ...
            <p className="text-gray-500 text-lg">{t('modules.clients.page.includedLead')}</p>
          </motion.div>
      ...
      <section className="py-20 md:py-28 bg-slate-900 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 max-w-5xl">
          <motion.div {...fadeUp} className="mb-12 md:mb-16">
            ...
            <p className="text-white/70 text-lg max-w-2xl">{t('modules.clients.page.scenariosLead')}</p>
          </motion.div>
            ...
            {SC_KEYS.map((k, i) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1 pt-1">
                  ...
                  <p className="text-white/90 leading-relaxed text-[15px] md:text-lg">{t(`modules.clients.${k}`)}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-16 bg-white">
        <div className="container mx-auto px-4">
          <motion.div {...fadeUp} className="max-w-2xl mb-10">
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">{t('modules.clients.page.moreModules')}</h2>
            <p className="text-gray-500">{t('modules.clients.page.moreModulesLead')}</p>
          </motion.div>
